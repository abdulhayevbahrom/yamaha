...
const isGwMlbbAutobuyEnabled = () => enabled(process.env.GW_MLBB_AUTOBUY_ENABLED);
...
function getCatalogMaxAgeMs() {
  return Math.max(
    60_000,
    Number(process.env.GW_MLBB_CATALOG_MAX_AGE_MS || 30 * 60_000),
  );
}
...
function providerTx(order, payload, extra = {}) {
  ...
  return {
    ...
    trxid: `YMH-MLBB-${String(order.orderId)}`,
...
async function complete(order, payload) {
  ...
  await awardReferralCommissionForOrder(updated).catch((error) => {
    console.error("GW MLBB referral error:", updated._id, error.message);
  });
...
function schedulePoll(orderId) {
  ...
  const delay = Math.max(3000, Number(process.env.GW_MLBB_POLL_INTERVAL_MS || 5000));
  const timer = setTimeout(() => {
    pollTimers.delete(key);
    pollGwMlbbOrder(key).catch((error) => console.error("GW MLBB poll error:", key, error.message));
  }, delay);
  timer.unref?.();
...
function scheduleSubmitRecovery(orderId) {
  const key = String(orderId);
  if (pollTimers.has(key)) return;
  const delay = Math.max(3_000, Number(process.env.GW_MLBB_POLL_INTERVAL_MS || 5_000));
  const timer = setTimeout(() => {
    pollTimers.delete(key);
    recoverGwMlbbSubmit(key).catch((error) =>
      console.error("GW MLBB submit recovery error:", key, error.message),
    );
  }, delay);
  timer.unref?.();
  pollTimers.set(key, timer);
...
async function handlePayload(order, payload) {
  ...
  const maxAttempts = Math.max(1, Number(process.env.GW_MLBB_POLL_MAX_ATTEMPTS || 24));
  await Order.findByIdAndUpdate(order._id, { $set: {
...
async function pollGwMlbbOrder(orderId) {
  if (!isGwMlbbAutobuyEnabled()) return { skipped: true, reason: "disabled" };
  ...
  try {
    ...
    if (isAmbiguousExternalError(error)) {
      ...
      const maxAttempts = Math.max(
        1,
        Number(process.env.GW_MLBB_POLL_MAX_ATTEMPTS || 24),
      );
...
async function recoverGwMlbbSubmit(orderId) {
  if (!isGwMlbbAutobuyEnabled()) return { skipped: true, reason: "disabled" };
  const order = await Order.findById(orderId);
  if (
    !order ||
  ...
  const maxAttempts = Math.max(
    1,
    Number(process.env.GW_MLBB_SUBMIT_RECOVERY_MAX_ATTEMPTS || 6),
  );
  const plan = await Plan.findOne({ category: "mlbb", code: order.planCode }).lean();
  ...
  const trxid = `YMH-MLBB-${String(order.orderId)}`;
  try {
    return handlePayload(order, await createOrder({
      pid: plan.providerProductId,
      userId: String(order.playerId || "").trim(),
      trxid,
      zoneId: String(order.zoneId || "").trim(), idempotencyKey: trxid,
    }));
  } catch (error) {
...
async function autoFulfillGwMlbb(orderOrId) {
  if (!isGwMlbbAutobuyEnabled()) return { skipped: true, reason: "disabled" };
  const claimed = await Order.findOneAndUpdate(
    { _id: orderOrId?._id || orderOrId, product: "mlbb", status: "paid_auto_processed", fulfillmentStatus: { $in: ["pending", "failed", "skipped"] } },
  ...
  const trxid = `YMH-MLBB-${String(claimed.orderId)}`;
  ...
  try {
    return handlePayload(claimed, await createOrder({
      pid: plan.providerProductId, userId: String(claimed.playerId || "").trim(),
      trxid, zoneId: String(claimed.zoneId || "").trim(), idempotencyKey: trxid,
    }));
  } catch (error) {
    const payload = error?.response?.data;
    if (payload && isDefinitiveSubmitFailure(error, payload)) {
...
async function resumeGwMlbbPolling() {
  if (!isGwMlbbAutobuyEnabled()) return 0;
  const rows = await Order.find({ product: "mlbb", fulfillmentStatus: "processing", "fragmentTx.provider": "gw" }).select({ _id: 1 }).lean();
