...
const isGwDeltaForceAutobuyEnabled = () => enabled(process.env.GW_DELTAFORCE_AUTOBUY_ENABLED);
...
const maxAge = () => Math.max(
  60_000,
  Number(process.env.GW_DELTAFORCE_CATALOG_MAX_AGE_MS || 30 * 60_000),
  Number(process.env.GW_DELTAFORCE_CATALOG_SYNC_INTERVAL_MS || 5 * 60_000) * 2,
);

function isGwDeltaForcePlanReady(plan) {
...
function tx(order, payload, extra = {}) {
  ...
  return {
    provider: "gw", providerVersion: "v1", productType: "deltaforce",
    providerOrderId: externalId(payload) || old.providerOrderId || "",
    providerStatus: statusOf(payload), trxid: `YMH-DELTAFORCE-${order.orderId}`,
...
async function complete(order, payload) {
  ...
  await awardReferralCommissionForOrder(updated).catch((error) => console.error("GW Delta Force referral error:", updated._id, error.message));
...
function schedule(orderId) {
  ...
  const timer = setTimeout(() => {
    timers.delete(key);
    pollGwDeltaForceOrder(key).catch((error) => console.error("GW Delta Force poll error:", key, error.message));
  }, Math.max(3000, Number(process.env.GW_DELTAFORCE_POLL_INTERVAL_MS || 5000)));
...
async function handle(order, payload) {
  ...
  const limit = Math.max(1, Number(process.env.GW_DELTAFORCE_POLL_MAX_ATTEMPTS || 24));
  const needsReview = attempts >= limit;
  await Order.findByIdAndUpdate(order._id, { $set: { fulfillmentStatus: needsReview ? "needs_review" : "processing", fulfillmentError: needsReview ? "GW Delta Force status polling timeout" : "", fragmentTx: tx(order, payload, { pollAttempts: attempts }) } });
  if (!needsReview) schedule(order._id);
  return { ok: false, processing: !needsReview, needsReview };
}

async function pollGwDeltaForceOrder(orderId) {
  if (!isGwDeltaForceAutobuyEnabled()) return { skipped: true, reason: "disabled" };
...
async function autoFulfillGwDeltaForce(orderOrId) {
  if (!isGwDeltaForceAutobuyEnabled()) return { skipped: true, reason: "disabled" };
  const order = await Order.findOneAndUpdate(
    { _id: orderOrId?._id || orderOrId, product: "deltaforce", status: "paid_auto_processed", fulfillmentStatus: { $in: ["pending", "failed", "skipped"] } },
  ...
  const plan = await Plan.findOne({ category: "deltaforce", code: order.planCode }).lean();
  if (!isGwDeltaForcePlanReady(plan)) {
    await Order.findByIdAndUpdate(order._id, { $set: { fulfillmentStatus: "needs_review", fulfillmentError: "GW Delta Force plan mapping unavailable" } });
    return { ok: false, needsReview: true };
  }
  ...
  try {
    return await handle(order, await createOrder({ pid: plan.providerProductId, userId: String(order.playerId || order.username || "").trim(), trxid: `YMH-DELTAFORCE-${order.orderId}`, idempotencyKey: `YMH-DELTAFORCE-${order.orderId}` }));
  } catch (error) {
    const payload = error?.response?.data;
    if ([400, 401, 403].includes(Number(error?.response?.status || 0)) && payload) return cancelAndRefund(order, payload);
    await Order.findByIdAndUpdate(order._id, { $set: { fulfillmentStatus: "needs_review", fulfillmentError: "GW Delta Force submit result unknown", fragmentTx: tx(order, payload, { phase: "submit_unknown", ambiguous: true }) } });
    return { ok: false, needsReview: true };
  }
}

async function resumeGwDeltaForcePolling() {
  if (!isGwDeltaForceAutobuyEnabled()) return 0;
  const rows = await Order.find({ product: "deltaforce", fulfillmentStatus: "processing", "fragmentTx.provider": "gw" }).select({ _id: 1 }).lean();
  rows.forEach((row) => schedule(row._id));
  return rows.length;
}

module.exports = { autoFulfillGwDeltaForce, pollGwDeltaForceOrder, resumeGwDeltaForcePolling, isGwDeltaForceAutobuyEnabled, isGwDeltaForcePlanReady };
