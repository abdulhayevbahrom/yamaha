...
const isGwGenshinAutobuyEnabled = () => enabled(process.env.GW_GENSHIN_AUTOBUY_ENABLED);
...
const maxAge = () => Math.max(60_000, Number(process.env.GW_GENSHIN_CATALOG_MAX_AGE_MS || 30 * 60_000));

function isGwGenshinPlanReady(plan) {
...
async function complete(order, payload) {
  ...
  await awardReferralCommissionForOrder(updated).catch((error) => console.error("GW Genshin referral error:", updated._id, error.message));
...
function schedule(orderId) {
  ...
  const timer = setTimeout(() => {
    timers.delete(key);
    pollGwGenshinOrder(key).catch((error) => console.error("GW Genshin poll error:", key, error.message));
...
async function handle(order, payload) {
  ...
  await Order.findByIdAndUpdate(order._id, { $set: { fulfillmentStatus: needsReview ? "needs_review" : "processing", fulfillmentError: needsReview ? "GW Genshin status polling timeout" : "", fragmentTx: tx(order, payload, { pollAttempts: attempts }) } });
  if (!needsReview) schedule(order._id);
  return { ok: false, processing: !needsReview, needsReview };
}

async function pollGwGenshinOrder(orderId) {
...
async function autoFulfillGwGenshin(orderOrId) {
  if (!isGwGenshinAutobuyEnabled()) return { skipped: true, reason: "disabled" };
  const order = await Order.findOneAndUpdate(
    { _id: orderOrId?._id || orderOrId, product: "genshin", status: "paid_auto_processed", fulfillmentStatus: { $in: ["pending", "failed", "skipped"] } },
