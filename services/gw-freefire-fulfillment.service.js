...
const isGwFreeFireAutobuyEnabled = () => enabled(process.env.GW_FREEFIRE_AUTOBUY_ENABLED);
...
const maxAge = () => Math.max(
  60_000,
  Number(process.env.GW_FREEFIRE_CATALOG_MAX_AGE_MS || 30 * 60_000),
  Number(process.env.GW_FREEFIRE_CATALOG_SYNC_INTERVAL_MS || 5 * 60_000) * 2,
);

function isGwFreeFirePlanReady(plan) {
...
function tx(order, payload, extra = {}) {
  ...
  return {
    provider: "gw", providerVersion: "v1", productType: "freefire",
    providerOrderId: externalId(payload) || old.providerOrderId || "",
    providerStatus: statusOf(payload), trxid: `YMH-FREEFIRE-${order.orderId}`,
...
async function complete(order, payload) {
  ...
  await awardReferralCommissionForOrder(updated).catch((error) => console.error("GW Free Fire referral error:", updated._id, error.message));
...
function schedule(orderId) {
  ...
  const timer = setTimeout(() => {
    timers.delete(key);
    pollGwFreeFireOrder(key).catch((error) => console.error("GW Free Fire poll error:", key, error.message));
  }, Math.max(3000, Number(process.env.GW_FREEFIRE_POLL_INTERVAL_MS || 5000)));
...
async function handle(order, payload) {
  ...
  const limit = Math.max(1, Number(process.env.GW_FREEFIRE_POLL_MAX_ATTEMPTS || 24));
  const needsReview = attempts >= limit;
  await Order.findByIdAndUpdate(order._id, { $set: { fulfillmentStatus: needsReview ? "needs_review" : "processing", fulfillmentError: needsReview ? "GW Free Fire status polling timeout" : "", fragmentTx: tx(order, payload, { pollAttempts: attempts }) } });
  if (!needsReview) schedule(order._id);
  return { ok: false, processing: !needsReview, needsReview };
}

async function pollGwFreeFireOrder(orderId) {
  if (!isGwFreeFireAutobuyEnabled()) return { skipped: true, reason: "disabled" };
...
async function autoFulfillGwFreeFire(orderOrId) {
  if (!isGwFreeFireAutobuyEnabled()) return { skipped: true, reason: "disabled" };
  const order = await Order.findOneAndUpdate(
    { _id: orderOrId?._id || orderOrId, product: "freefire", status: "paid_auto_processed", fulfillmentStatus: { $in: ["pending", "failed", "skipped"] } },
  ...
  const plan = await Plan.findOne({ category: "freefire", code: order.planCode }).lean();
  if (!isGwFreeFirePlanReady(plan)) {
    await Order.findByIdAndUpdate(order._id, { $set: { fulfillmentStatus: "needs_review", fulfillmentError: "GW Free Fire plan mapping unavailable" } });
    return { ok: false, needsReview: true };
  }
  ...
  try {
    return await handle(order, await createOrder({ pid: plan.providerProductId, userId: String(order.playerId || order.username || "").trim(), trxid: `YMH-FREEFIRE-${order.orderId}`, idempotencyKey: `YMH-FREEFIRE-${order.orderId}` }));
  } catch (error) {
    const payload = error?.response?.data;
    if ([400, 401, 403].includes(Number(error?.response?.status || 0)) && payload) return cancelAndRefund(order, payload);
    await Order.findByIdAndUpdate(order._id, { $set: { fulfillmentStatus: "needs_review", fulfillmentError: "GW Free Fire submit result unknown", fragmentTx: tx(order, payload, { phase: "submit_unknown", ambiguous: true }) } });
    return { ok: false, needsReview: true };
  }
}

async function resumeGwFreeFirePolling() {
  if (!isGwFreeFireAutobuyEnabled()) return 0;
  const rows = await Order.find({ product: "freefire", fulfillmentStatus: "processing", "fragmentTx.provider": "gw" }).select({ _id: 1 }).lean();
  rows.forEach((row) => schedule(row._id));
  return rows.length;
}

module.exports = { autoFulfillGwFreeFire, pollGwFreeFireOrder, resumeGwFreeFirePolling, isGwFreeFireAutobuyEnabled, isGwFreeFirePlanReady };
